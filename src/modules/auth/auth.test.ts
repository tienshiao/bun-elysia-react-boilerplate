...
import { makeAuthGuard, makeAuthPlugin } from "./index.ts";
import { makeJwt } from "./jwt.ts";
import { AuthenticatedRole, makeAllowRoles } from "./roles.ts";
...
const apiPlugin = new Elysia({ prefix: "/api" }).use(makeAuthPlugin(testDb, jwt));
...
describe("Auth API", () => {
  ...
  describe("POST /api/auth/refresh", () => {
    ...
    it("returns new refresh_token when near expiry threshold", async () => {
      ...
      await testDb.execute(sql`UPDATE refresh_tokens SET expires_at = now() + interval '1 day'`);
  ...
  describe("Auth Guard + allowRoles", () => {
    ...
    it("returns 403 without Authorization header", async () => {
      const res = await testApp.handle(new Request("http://localhost/api/me"));
